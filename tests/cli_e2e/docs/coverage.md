...
## Summary
- TestDocs_CreateAndFetchWorkflowAsBot: proves `docs +create` and `docs +fetch`; reads a document larger than 24 KiB with explicit pagination, then verifies the default complete read automatically uses temporary-file delivery through `content_file`, including its tail content.
...
| ✓ | docs +fetch | shortcut | docs_fetch_dryrun_test.go; docs_create_fetch_test.go::TestDocs_CreateAndFetchWorkflowAsBot/fetch paginated + fetch default complete with automatic spill; docs_update_test.go::TestDocs_UpdateWorkflow/verify as bot; docs_create_fetch_test.go::TestDocs_CreateAndFetchWorkflowAsUser/fetch as user; docs_update_dryrun_test.go::TestDocs_DryRunDefaultsToV2OpenAPI/fetch | `--doc <docToken>`; `--doc-format markdown`; default complete read; `--paginate`; `--page-token`; `--page-size`; default `extra_param.enable_user_cite_reference_map=true`; `--api-version v1` compatibility flag still dry-runs the v2 fetch endpoint | dry-run proves complete reads are the default and pagination is explicit; live default-complete case verifies stdout omits `content`, returns a temporary `content_file`, and preserves the fixture tail in the saved Markdown |
