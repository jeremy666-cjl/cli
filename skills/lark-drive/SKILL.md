...
- 只需要**查看网盘普通文件的内容**（PDF / Word / Excel / 附件等，用于阅读 / 总结），用 `lark-cli drive +fetch`——返回可读 markdown；要把**原始文件字节存到本地**才用 `lark-cli drive +download`。在线文档（docx / sheet / bitable / wiki）的内容仍走 `docs +fetch --api-version v2`。
...
| [`+fetch`](references/lark-drive-fetch.md) | Fetch a Drive file's readable content as markdown; 读内容用 `+fetch`，取原始文件字节用 `+download`。 |
