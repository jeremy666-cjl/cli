...
| 读整表为 markdown 正文 | `+fetch` | 把整个多维表读成一串可读 markdown 正文（各子表展开成 GFM 表格，供喂模型 / 总结 / 阅读）；走 qa fetch，URL 带 `?table=` 自动选子表；要结构化 record / 明细 / 分析仍用 `+record-list` / `+record-get` / `+data-query`，qa 不可用时 `+fetch` 直接报错并提示改用 `+record-list`。读 [lark-base-fetch.md](references/lark-base-fetch.md) |
...
- [lark-base-fetch.md](references/lark-base-fetch.md)：把整张多维表读成一串可读 markdown 正文（`+fetch`，走 qa fetch）
