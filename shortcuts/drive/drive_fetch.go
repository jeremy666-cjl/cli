...
func runDriveFetch(ctx context.Context, runtime *common.RuntimeContext) error {
	...
	if in.inputType == "wiki" {
		...
		if werr != nil {
			...
			// scope, or the node is not found). eqa reads the wiki URL directly
			// server-side (it unwraps the node itself), so fall back to forwarding
			// the raw wiki URL to eqa fetch instead of failing the whole read.
			fmt.Fprintf(runtime.IO().ErrOut,
				"[fetch] wiki get_node failed (%v); falling back to direct eqa fetch of the wiki URL\n", werr)
			...
			return emitDriveFetch(runtime, out, res, render)
		}
...
// the envelope shape (content/resource/render/warnings + debug backend on
// stderr) stays identical.
func emitDriveFetch(runtime *common.RuntimeContext, out *driveFetchOutput, res fetchResource, render fetchRender) error {
...
// (resource.url): for wiki input the underlying resource with the original
// ?sheet=/?table=/?view= selectors reattached (mirroring forwardResourceURL, so
// the recorded URL reads the same sub-resource eqa was asked to); the input URL
// verbatim for URL input; a built /<type>/<token> for bare tokens.
