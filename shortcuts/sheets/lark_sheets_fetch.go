...
func validateSheetFetch(ctx context.Context, runtime *common.RuntimeContext) error {
	...
	if url := strings.TrimSpace(runtime.Str("url")); url != "" {
		...
		if ref, ok := common.ParseResourceURL(url); !ok || (ref.Type != "sheet" && ref.Type != "wiki") {
			return common.ValidationErrorf("--url must be a spreadsheet URL (https://.../sheets/<token>) or a wiki link to a sheet")
		}
	}
	return nil
}
...
func sheetFetchUnavailable(cause error) error {
	return errs.NewAPIError(errs.SubtypeServerError,
		"qa fetch unavailable (%v); for spreadsheet data use `lark-cli sheets +cells-get --url <url> --range <A1:..>` or `sheets +workbook-info`",
		cause)
}
