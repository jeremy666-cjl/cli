...
// Path is the knowledge-qa content fetch route (materialized markdown, or for
...
// (wiki unwrap, minutes transcript) are ensured by the dispatcher lane that
// needs them.
func FetchDocInfo(ctx context.Context, runtime *common.RuntimeContext, req Request) (*Response, error) {
	...
	if err != nil {
		return nil, errs.NewInternalError(errs.SubtypeUnknown, "marshal fetch data: %s", err)
	}
	var resp Response
	if err := json.Unmarshal(raw, &resp); err != nil {
		return nil, errs.NewInternalError(errs.SubtypeUnknown, "decode fetch response: %s", err)
	}
	return &resp, nil
}
