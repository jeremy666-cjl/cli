...
// node_token; passing it makes the intent unambiguous and matches the
// previously-validated probe path. Returns a typed InvalidResponse error when
// get_node succeeds but the node lacks obj_type/obj_token (incomplete data);
// transport/permission errors come back as the underlying CallAPITyped error
// for the caller to annotate.
