...
func TestCitationInjectedWhenEnabled(t *testing.T) {
...
func TestCitationByteIdenticalWhenDisabled(t *testing.T) {
	called := false
...
func TestCitationMountPanics(t *testing.T) {
...
// TestCitationOutRawInjectedWhenEnabled mirrors
// TestCitationInjectedWhenEnabled for RuntimeContext.OutRaw, the
...
// stand-in.
func TestCitationOutRawInjectedWhenEnabled(t *testing.T) {
	t.Setenv(envvars.CliCitation, "1")
	called := false
...
// TestCitationOutFormatRawInjectedWhenEnabled mirrors
// TestCitationInjectedWhenEnabled for RuntimeContext.OutFormatRaw, the
// HTML-escaping-disabled sibling of OutFormat, proving the same
// citationProvider wiring on the --format-aware raw path.
func TestCitationOutFormatRawInjectedWhenEnabled(t *testing.T) {
	t.Setenv(envvars.CliCitation, "1")
	called := false
	s := citationTestShortcut(&called)
	s.Execute = func(ctx context.Context, rt *RuntimeContext) error {
...
// TestCitationTableFormatDoesNotBuild pins that OutFormat's "table"
...
func TestCitationTableFormatDoesNotBuild(t *testing.T) {
	t.Setenv(envvars.CliCitation, "1")
	called := false
	s := &Shortcut{
