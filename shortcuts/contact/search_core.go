...
func SearchUsersCore(ctx context.Context, runtime *common.RuntimeContext, p SearchUsersParams) (SearchUsersResult, error) {
	...
	if err != nil {
		return SearchUsersResult{}, fmt.Errorf("contact.SearchUsersCore: %w", err)
	}
	if apiResp.StatusCode != http.StatusOK {
		return SearchUsersResult{}, output.ErrAPI(apiResp.StatusCode, http.StatusText(apiResp.StatusCode), string(apiResp.RawBody))
	}
	var env searchUserAPIEnvelope
	if err := json.Unmarshal(apiResp.RawBody, &env); err != nil {
		return SearchUsersResult{}, fmt.Errorf("contact.SearchUsersCore: unmarshal: %w", err)
	}
	if env.Code != 0 {
		return SearchUsersResult{}, output.ErrAPI(env.Code, env.Msg, string(apiResp.RawBody))
	}
	users, hasMore := projectUsers(env.Data, "", runtime.Config.Brand)
