...
var MinutesFetch = common.Shortcut{
	...
	Validate: func(_ context.Context, runtime *common.RuntimeContext) error {
		...
		if token == "" || !validMinuteToken.MatchString(token) {
			return output.ErrValidation("invalid --minute-token %q: pass a minutes URL or a bare token (lowercase alphanumeric)", raw)
		}
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		metaData, err := runtime.DoAPIJSON(http.MethodGet,
		...
		if minute == nil {
			return output.Errorf(output.ExitAPI, "not_found", "minute not found: %s", token)
		}
		...
		art, err := runtime.DoAPIJSON(http.MethodGet,
		...
		if includes["transcript"] {
			if txt, ok := fetchTranscriptText(ctx, runtime, token); ok {
...
func parseMinutesIncludes(raw string) (map[string]bool, error) {
	...
	for _, v := range common.SplitCSV(raw) {
		if !minutesFetchIncludes[v] {
			return nil, output.ErrValidation("invalid --include value %q (allowed: transcript, note-doc)", v)
		}
		set[v] = true
	}
	return set, nil
