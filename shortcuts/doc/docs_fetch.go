...
func useV2Fetch(runtime *common.RuntimeContext) bool {
	...
	for _, name := range []string{"detail", "doc-format", "scope", "revision-id", "start-block-id", "end-block-id", "keyword", "context-before", "context-after", "max-depth", "inline-embeds", "embed-max-rows", "image-urls"} {
