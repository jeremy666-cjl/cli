...
func NewClient() (*Client, error) {
	base := BaseURL()
	if base == "" {
		return nil, output.ErrWithHint(output.ExitInternal, "config",
			faasBaseEnv+" not set; the faas gateway URL is required",
			fmt.Sprintf("export %s=https://<faas-host>", faasBaseEnv))
	}
	return &Client{
		baseURL: base,
...
func (c *Client) PostJSON(ctx context.Context, ident Identity, path string, body any) ([]byte, error) {
	...
	if err != nil {
		return nil, output.Errorf(output.ExitInternal, "internal", "marshal faas request: %s", err)
	}
