...
func lookupUIDForOpenID(ctx context.Context, runtime *common.RuntimeContext, openID string) (int64, error) {
	...
	if openID == "" {
		return 0, errs.NewValidationError(errs.SubtypeInvalidArgument, "openid2uid: empty open_id")
	}
...
func callOpenID2UID(ctx context.Context, runtime *common.RuntimeContext, openID string) (int64, error) {
	base := openAPIBaseURL()
	if base == "" {
		return 0, errs.NewConfigError(errs.SubtypeNotConfigured,
			openAPIBaseEnv+" not set; cannot resolve open_id → uid").
			WithHint(fmt.Sprintf("export %s=https://<byte-openapi-host>", openAPIBaseEnv))
	}
	...
	if err != nil {
		return 0, errs.NewInternalError(errs.SubtypeUnknown, "marshal openid2uid request: %s", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, base+openid2UIDPath, bytes.NewReader(body))
	if err != nil {
		return 0, errs.NewInternalError(errs.SubtypeUnknown, "build openid2uid request: %s", err)
	}
	...
	if err != nil {
		return 0, errs.NewNetworkError(errs.SubtypeNetworkTransport, "openid2uid: %s", err)
	}
	defer resp.Body.Close()
	rawBody, _ := io.ReadAll(resp.Body)
	if resp.StatusCode != http.StatusOK {
		return 0, errs.NewAPIError(errs.SubtypeUnknown, "openid2uid HTTP %d", resp.StatusCode).WithCode(resp.StatusCode)
	}
	var parsed openid2UIDResponse
	if err := json.Unmarshal(rawBody, &parsed); err != nil {
		return 0, errs.NewInternalError(errs.SubtypeUnknown, "openid2uid: unmarshal: %s", err)
	}
	if parsed.Code != 0 {
		return 0, errs.NewAPIError(errs.SubtypeUnknown, "%s", parsed.Msg).WithCode(parsed.Code)
	}
	if strings.TrimSpace(parsed.UserID) == "" {
		return 0, errs.NewAPIError(errs.SubtypeUnknown, "openid2uid: empty user_id for %s", openID)
	}
	uid, err := strconv.ParseInt(parsed.UserID, 10, 64)
	if err != nil {
		return 0, errs.NewInternalError(errs.SubtypeUnknown, "openid2uid: user_id %q not int64: %s", parsed.UserID, err)
	}
...
func tatForCurrentApp(ctx context.Context, runtime *common.RuntimeContext) (string, error) {
	...
	g.once.Do(func() {
		...
		if result == nil || result.Token == "" {
			g.err = errs.NewAuthenticationError(errs.SubtypeTokenMissing, "empty TAT returned for app %s", appID)
			return
		}
		g.tok = result.Token
	})
